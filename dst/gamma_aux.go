...
const M_cutoff = 3.196577e18
const DBL_EPSILON = 2.2204460492503131e-16
const DBL_MIN = 2.225074e-308
...
// R_Log1_Exp()
func R_Log1_Exp(x float64) float64 {
	if x > -Ln2 {
		return log(-expm1(x))
	}
	return log1p(-exp(x))
...
func pd_upper_series(x, y float64) float64 {
	...
	for term > sum*DBL_EPSILON { // was do-while loop, OK???
		y++
		term *= x / y
		sum += term
	}
...
func pgamma_raw(x, shape float64) float64 {
	...
	*/
	...
	if res < DBL_MIN/DBL_EPSILON {
		/* with(.Machine, float64.xmin / float64.eps) #|-> 1.002084e-292 */
