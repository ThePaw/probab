...
// Test against R:moments:bonett.test

func TestSampleBonett(t *testing.T) {
	const delta = 1e-4
	...
	fmt.Println("Bonett test two-sided")
	alt := 0
	a, b, c := Bonett(d, alt)
	x, y, z := 0.8074, 0.9789, 0.3276
	...
	fmt.Println("Bonett test less")
	alt = 1
	a, b, c = Bonett(d, alt)
	x, y, z = 0.8074, 0.9789, 0.1638
	...
	fmt.Println("Bonett test greater")
	alt = 2
	a, b, c = Bonett(d, alt)
	x, y, z = 0.8074, 0.9789, 0.8362
	if abs(x-a) > delta {
		fmt.Println("failed skewness: ", a, x)
		t.Error()
	}
